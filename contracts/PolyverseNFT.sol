// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";

contract PolyverseNFT is ERC721 {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor(string memory creatorName) ERC721(creatorName, "PVNFT") {}

    function mintTicket(address owner, uint256 ticketId) external {
        _tokenIds.increment();
        uint256 newTicketId = _tokenIds.current();
        _mint(owner, newTicketId);
    }

    function mintSubscription(address subscriber) external {
        _tokenIds.increment();
        uint256 newSubscriptionId = _tokenIds.current();
        _mint(subscriber, newSubscriptionId);
    }
}
